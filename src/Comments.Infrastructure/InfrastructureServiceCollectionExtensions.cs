...
/// <summary>Registers everything that talks to the outside world: SQL, Redis, Elasticsearch, Blob Storage, RabbitMQ.</summary>
public static class InfrastructureServiceCollectionExtensions
{
    ...
    public static IServiceCollection AddPersistence(
        ...
        IConfiguration configuration)
    {
        var connectionString = ConnectionStrings.GetRequired(configuration, ConnectionStrings.SqlServer);
    ...
    {
        var redisConnection = ConnectionStrings.GetRequired(configuration, ConnectionStrings.Redis);
...
#pragma warning restore EXTEXP0018
        ...
        Action<IBusRegistrationConfigurator>? configureConsumers = null)
    {
        ...
        var rabbit = ConnectionStrings.GetRequired(configuration, ConnectionStrings.RabbitMq);
    ...
    /// <summary>
    /// Registers the one-time startup work. Only the API applies migrations; see
    /// <see cref="InfrastructureInitializer"/>.
    /// </summary>
    public static IServiceCollection AddInfrastructureInitializer(this IServiceCollection services, bool applyMigrations)
    {
        services.AddHostedService(provider =>
            ActivatorUtilities.CreateInstance<InfrastructureInitializer>(provider, applyMigrations));

        return services;
    }

    /// <summary>Registers the background service that drains the outbox onto the broker.</summary>
